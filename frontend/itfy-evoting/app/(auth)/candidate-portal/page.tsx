...
import type { Candidate, Category, ApiResponse } from '@/types';
...
function ShareProfileDialog({ candidate }: { candidate: Candidate }) {
  const profileUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/candidates/${candidate.slug}`
    : '';
...
function CandidatePortalContent() {
  ...
  const { candidate: authCandidate, logout, isLoading: authLoading } = useCandidateAuth();
                ...
                <CardContent>
                  {candidate.categories && candidate.categories.length > 0 ? (
                    <div className="space-y-3">
                      {candidate.categories.map((categoryId, index) => (
                        <div
                          key={categoryId}
