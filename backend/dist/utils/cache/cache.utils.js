...
class CacheManager {
  ...
  async connect() {
    ...
    try {
      console.log(`Connecting to Redis: ${this.redisUrl.replace(/:[^:@]+@/, ":***@")}`);
  ...
  async deletePattern(pattern) {
    try {
      ...
      console.warn(`Pattern deletion failed (${pattern}):`, error.message);
      return 0;
    }
  }
  ...
  async getKeys(pattern) {
    try {
      ...
      let cursor = "0";
      do {
        const [newCursor, matchedKeys] = await this.client.scan(cursor, "MATCH", pattern, "COUNT", 100);
        cursor = newCursor;
        keys.push(...matchedKeys);
      } while (cursor !== "0");
      return keys;
    } catch (error) {
      console.warn(`Get keys failed (${pattern}):`, error.message);
      return [];
    }
  }
