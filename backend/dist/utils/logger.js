...
const SENSITIVE_FIELDS = ["password", "password_hash", "token", "accessToken", "refreshToken", "authorization", "api_key", "apiKey", "secret", "credit_card", "creditCard", "ssn", "cvv"];
 ...
 */
const devFormat = winston.format.combine(winston.format.timestamp(), winston.format.colorize({
  all: true
 ...
 */
const prodFormat = winston.format.combine(winston.format.timestamp(), winston.format.errors({
  stack: true
}), winston.format(info => {
