  ...
  return (req, res, next) => {
    ...
    res.send = function (data) {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        const description = options.getDescription ? options.getDescription(req, res) : `${action} completed successfully`;

        // Generate metadata
        const metadata = options.getMetadata ? options.getMetadata(req, res) : {};

        // Extract event context if available
        const eventId = req.params.eventId || req.body && req.body.event_id || res.locals.eventId || null;
        ...
        agendaManager.now("log-activity", {
...
export const logDataChange = (action, entityType, getChanges) => {
  return async (req, res, next) => {
    ...
    res.send = function (data) {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        // Capture "after" state
        Promise.resolve(getChanges(req, "after", res)).then(afterData => {
          ...
          agendaManager.now("log-activity", {
            ...
            console.error(`[Activity Logger] Failed to log data change: ${error.message}`);
          });
        }).catch(error => {
...
export const batchLogActivities = async activities => {
  try {
    await agendaManager.now("batch-log-activities", {
      activities
    });
  } catch (error) {
