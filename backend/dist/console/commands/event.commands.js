...
export class EventCommands {
  ...
  async listEvents(args) {
    ...
    try {
      ...
      const [events, total] = await Promise.all([Event.find(query).select('name status start_date end_date voting_start voting_end is_featured createdAt').sort({
        createdAt: -1
      ...
      const rows = events.map(event => [event.name.substring(0, 30), event.status, event.start_date ? new Date(event.start_date).toLocaleDateString() : 'N/A', event.end_date ? new Date(event.end_date).toLocaleDateString() : 'N/A', event.is_featured ? 'Yes' : 'No']);
  ...
  async viewEvent(identifier) {
    ...
    try {
      ...
      const event = await Event.findOne(query).populate('organiser', 'name email').lean();
      if (!event) {
        spinner.stop();
  ...
  async stats() {
    ...
    try {
      const Event = await this.getEventModel();
      const [totalEvents, activeEvents, draftEvents, completedEvents, featuredEvents] = await Promise.all([Event.countDocuments(), Event.countDocuments({
        status: 'active'
      ...
      // Recent events
      const recentEvents = await Event.find().select('name status createdAt').sort({
        createdAt: -1
      }).limit(5).lean();
