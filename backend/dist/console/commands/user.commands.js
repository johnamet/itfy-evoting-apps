...
export class UserCommands {
  ...
  async listUsers(args) {
    ...
    try {
      ...
      const [users, total] = await Promise.all([User.find(query).select('name email role status last_login createdAt').sort({
        createdAt: -1
      ...
      const rows = users.map(user => [user.name, user.email, user.role, user.status || 'active', user.last_login ? new Date(user.last_login).toLocaleString() : 'Never']);
  ...
  async createUser() {
    ...
    while (!passwordMatch) {
      ...
      const confirm = await this.ui.question('Confirm password', {
        hidden: true
      });
  ...
  async editUser(identifier) {
    ...
    try {
      ...
      this.ui.newLine();
      const updates = {};
      const newName = await this.ui.question('Name', {
        defaultValue: user.name
      });
      if (newName !== user.name) updates.name = newName;
      const newEmail = await this.ui.question('Email', {
        defaultValue: user.email
      });
      if (newEmail !== user.email) updates.email = newEmail;
      const newStatus = await this.ui.select('Status:', [{
        value: user.status || 'active',
        label: `${user.status || 'active'} (current)`
      }, {
        value: 'active',
        label: 'Active'
      }, {
