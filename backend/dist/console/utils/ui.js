...
${colors.reset}`;
  ...
  async question(prompt, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      const promptText = defaultValue ? `${colors.yellow}${icons.arrow}${colors.reset} ${prompt} ${colors.dim}[${defaultValue}]${colors.reset}: ` : `${colors.yellow}${icons.arrow}${colors.reset} ${prompt}: `;
      if (hidden) {
  ...
  async confirm(message, defaultValue = false) {
    ...
    const answer = await this.question(`${message} ${hint}`, {
