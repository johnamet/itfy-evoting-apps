...
class FormService extends BaseService {
  ...
  async createForm(formData, createdBy) {
    try {
      ...
      if (validated.form_type === FORM_TYPE.NOMINATION && validated.candidate_field_mapping?.enabled) {
  ...
  extractCandidateData(submissionData, mappings) {
    ...
    const data = submissionData instanceof Map ? Object.fromEntries(submissionData) : submissionData;
    for (const mapping of mappings) {
      const {
        form_field_id,
        candidate_field,
