...
class FormSubmission extends BaseModel {
  constructor() {
      ...
      try {
        ...
        if (!this.nominee_identifier && this.submission_data) {
          ...
          for (const field of identifierFields) {
            ...
            if (value) {
              identifierParts.push(String(value).toLowerCase().trim());
            }
          }
    ...
    this.schema.post("save", async function (doc) {
      try {
        ...
        if (form) {
          ...
          const totalSubmissions = await doc.constructor.countDocuments({
            form: doc.form
          });
    ...
    this.schema.statics.checkForDuplicates = async function (formId, submissionData, checkFields, threshold = 85) {
      const Form = mongoose.model("Form");
      ...
      for (const existing of existingSubmissions) {
        ...
        for (const fieldId of fieldsToCheck) {
          const newValue = submissionData.get ? submissionData.get(fieldId) : submissionData[fieldId];
          const existingValue = existing.submission_data.get ? existing.submission_data.get(fieldId) : existing.submission_data[fieldId];
