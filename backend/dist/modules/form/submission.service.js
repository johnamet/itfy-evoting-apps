...
class SubmissionService extends BaseService {
  ...
  async submitForm(formId, submissionData, metadata = {}) {
    try {
      ...
      if (form.multi_category_nomination?.enabled) {
        const identifierFields = form.fields.filter(f => f.is_identifier_field).map(f => submissionData[f.field_id] || "").join("|");
        nomineeIdentifier = crypto.createHash("sha256").update(identifierFields.toLowerCase()).digest("hex");
      }
  ...
  async checkDuplicates(submissionId, threshold = null) {
    try {
      ...
      // Run duplicate check
      const duplicates = await this.repository.checkDuplicates(form._id, submission.submission_data, checkThreshold);
  ...
  async approveSubmission(submissionId, adminId, options = {}) {
    try {
      ...
      if (form.form_type === FORM_TYPE.NOMINATION && form.candidate_field_mapping?.enabled && (options.createCandidate ?? form.candidate_field_mapping.auto_create_candidate)) {
        // Extract candidate data using field mapping
        const candidateData = FormService.extractCandidateData(submission.submission_data, form.candidate_field_mapping.mappings);
        ...
        candidate = await CandidateService.createFromNomination(finalCandidateData, submission.event._id || submission.event, submission.categories.map(c => c._id || c), submissionId, adminId // Track which admin approved and created the candidate
        );
  ...
  async resolveDuplicate(submissionId, adminId, resolution) {
    try {
      ...
      switch (resolution.action) {
        ...
        case "reject":
          // Reject as duplicate
          result = await this.rejectSubmission(submissionId, adminId, resolution.notes || "Rejected as duplicate");
          break;
        case "merge":
          ...
          result = await this.repository.markAsDuplicate(submissionId, adminId, [resolution.mergeWith], submission.duplicate_check.similarity_score);
