...
import { FORM_TYPE, FORM_STATUS, FIELD_TYPE, DUPLICATE_CHECK_METHOD } from "../../utils/constants/form.constants.js";
...
const fieldMappingItemSchema = Joi.object({
  form_field_id: Joi.string().trim().required().messages({
    "string.empty": "Form field ID is required"
  }),
  candidate_field: Joi.string().trim().required().messages({
...
class FormValidation {
  ...
  static createFormSchema = Joi.object({
    ...
    form_type: Joi.string().valid(...Object.values(FORM_TYPE)).required().messages({
    ...
    close_date: Joi.date().when("open_date", {
      is: Joi.exist(),
      then: Joi.date().greater(Joi.ref("open_date")).messages({
  ...
  static updateFormSchema = Joi.object({
    name: Joi.string().trim().min(3).max(200).messages({
      "string.min": "Form name must be at least 3 characters",
      "string.max": "Form name cannot exceed 200 characters"
    }),
    ...
    form_type: Joi.string().valid(...Object.values(FORM_TYPE)).messages({
      "any.only": `Form type must be one of: ${Object.values(FORM_TYPE).join(", ")}`
    }),
    categories: Joi.array().items(ObjectId()),
    fields: Joi.array().items(formFieldSchema).min(1).messages({
      "array.min": "At least one field is required"
    }),
    candidate_field_mapping: candidateFieldMappingSchema,
    duplicate_detection: duplicateDetectionSchema,
  ...
  static updateFieldMappingSchema = Joi.object({
    mappings: Joi.array().items(fieldMappingItemSchema).min(1).required().messages({
  ...
  static formQuerySchema = Joi.object({
    ...
    sort: Joi.string().valid("name", "total_submissions", "created_at", "-name", "-total_submissions", "-created_at").default("-created_at")
  });
  ...
  static updateFormStatusSchema = Joi.object({
    status: Joi.string().valid(...Object.values(FORM_STATUS)).required().messages({
