...
import { FORM_TYPE, FORM_STATUS as STATUS, FIELD_TYPE, SUBMISSION_STATUS, DUPLICATE_CHECK_METHOD } from "../../utils/constants/form.constants.js";
class Form extends BaseModel {
  constructor() {
    ...
    this.schema.pre("save", async function (next) {
      try {
        ...
        if (this.duplicate_detection.enabled && this.duplicate_detection.check_fields.length === 0) {
          const checkFields = this.fields.filter(f => f.is_duplicate_check_field).map(f => f.field_id);
        ...
        if (this.multi_category_nomination.enabled && this.form_type !== FORM_TYPE.NOMINATION) {
    ...
    this.schema.virtual("submissionRate").get(function () {
      ...
      const now = new Date();
      const daysSinceOpen = Math.max(1, Math.ceil((now - this.open_date) / (1000 * 60 * 60 * 24)));
    ...
    this.schema.statics.findByType = async function (formType, eventId = null, options = {}) {
      ...
      const query = this.find(filter).sort({
        created_at: -1
      });
      if (options.populate) query.populate(options.populate);
      return await query.exec();
    ...
    this.schema.methods.addField = function (fieldData) {
      const fieldId = fieldData.field_id || `field_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      this.fields.push({
        ...fieldData,
        field_id: fieldId,
        ...
        if (value !== null && value !== undefined && value !== "") {
          ...
          if (field.validation.pattern) {
            ...
            if (!regex.test(value)) {
              errors.push({
                field_id: field.field_id,
                message: field.validation.custom_message || `${field.label} does not match required format`
              });
            }
          }
        }
