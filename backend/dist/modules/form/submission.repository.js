...
class FormSubmissionRepository extends BaseRepository {
  ...
  async getFormStatistics(formId) {
    try {
      ...
      return {
        ...
        avgDuplicateSimilarity: Math.round((stats?.avgSimilarity[0]?.average || 0) * 10) / 10,
  ...
  async checkDuplicates(formId, submissionData, threshold = 80) {
    try {
      const form = await this.model.db.collection("forms").findOne({
        _id: formId
      });
      if (!form || !form.duplicate_detection?.enabled) {
        return [];
      }
      const duplicates = await FormSubmissionModel.checkForDuplicates(formId, submissionData, threshold);
      return duplicates;
    } catch (error) {
  ...
  async markAsDuplicate(submissionId, reviewerId, matchedSubmissions, similarityScore) {
    try {
      ...
      return await submission.markAsDuplicate(reviewerId, matchedSubmissions, similarityScore);
    } catch (error) {
  ...
  async bulkApprove(submissionIds, reviewerId, notes = "") {
    try {
      const result = await this.model.updateMany({
        ...
        status: {
          $in: [STATUS.PENDING, STATUS.UNDER_REVIEW]
        }
      }, {
        $set: {
          status: STATUS.APPROVED,
   ...
   * @returns {Promise<Object>} - Paginated submissions
   */
  async findByDateRange(startDate, endDate, formId = null, page = 1, limit = 20, options = {}) {
    try {
      const filter = {
        created_at: {
          $gte: startDate,
  ...
  async exportSubmissions(formId, filters = {}) {
    try {
      ...
      const submissions = await this.model.find(query).populate("category", "name").populate("approval.approved_by", "name email").sort({
        created_at: -1
      }).lean().exec();
