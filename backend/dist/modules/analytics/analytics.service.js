...
class AnalyticsService {
  ...
  async getEventDashboard(eventId) {
    try {
      const [candidates, categories, votes, bundles, coupons, forms, submissions, payments] = await Promise.all([CandidateRepository.getStatisticsByEvent(eventId), CategoryRepository.getStatisticsByEvent(eventId), VoteRepository.getStatisticsByEvent(eventId), BundleRepository.getStatisticsByEvent(eventId), CouponRepository.getStatisticsByEvent(eventId), FormRepository.getStatistics(eventId), FormSubmissionRepository.getEventStatistics(eventId), PaymentModel.getStatistics(eventId)]);

      // Calculate derived metrics
      const conversionRate = submissions.total > 0 ? votes.totalVotes / submissions.total * 100 : 0;
      const averageRevenuePerVote = votes.totalVotes > 0 ? payments.totalRevenue / votes.totalVotes : 0;
      const roi = {
  ...
  async getDashboardOverview(params = {}) {
    try {
      ...
      const [eventStats, userCount, candidateCount, categoryCount, voteStats, paymentStats, systemHealth] = await Promise.all([EventRepository.getStatistics(), UserModel.countDocuments({
        deleted_at: null
      }), CandidateRepository.model.countDocuments({
        deleted_at: null
      }), CategoryRepository.model.countDocuments({
        deleted_at: null
      }), this._getVoteStatistics(), this._getPaymentStatistics(), healthService.getSystemHealth()]);
      ...
      const overallParticipationRate = this._calculateParticipationRate(voteStats.totalVotes, eventStats.active, candidateCount);
      return {
  ...
  async _getVoteStatistics() {
    try {
      ...
      const eventCount = await EventRepository.model.countDocuments({
        deleted_at: null
      });
      return {
  ...
  async _getPaymentStatistics() {
    try {
      ...
      const successfulPayments = stats.find(s => s._id === "completed") || {
        total: 0,
        count: 0
      };
  ...
  async _calculateGrowthRates(period = "month") {
    try {
      ...
      const [currentUsers, currentEvents, currentVotes, currentRevenue] = await Promise.all([UserModel.countDocuments({
        created_at: {
          $gte: currentStart
        },
        deleted_at: null
      }), EventRepository.model.countDocuments({
        created_at: {
          $gte: currentStart
        },
        deleted_at: null
      ...
      const [previousUsers, previousEvents, previousVotes, previousRevenue] = await Promise.all([UserModel.countDocuments({
        created_at: {
          $gte: previousStart,
          $lt: previousEnd
        },
      ...
      return {
        ...
        votes: calculateGrowth(currentVotes[0]?.total || 0, previousVotes[0]?.total || 0),
        revenue: calculateGrowth(currentRevenue[0]?.total || 0, previousRevenue[0]?.total || 0)
      };
    } catch (error) {
  ...
  async getPlatformDashboard() {
    try {
      const [events, forms, bundles, categories, coupons] = await Promise.all([EventRepository.getStatistics(), FormRepository.getStatistics(), BundleRepository.getOverallStatistics(), CategoryRepository.getOverallStatistics(), CouponRepository.getOverallStatistics()]);
      return {
        events,
        forms,
        bundles,
  ...
  async getPaymentAnalytics(params = {}) {
    try {
      ...
      const averageTransactionValue = stats.totalTransactions > 0 ? stats.totalRevenue / stats.totalTransactions : 0;
      return {
  ...
  async getUserEngagement(eventId, startDate, endDate) {
    try {
      const activities = await ActivityRepository.findByTimeRange(startDate, endDate, {
        event: eventId
      }, 1, 10000);
  ...
  async compareEvents(eventIds) {
    try {
      const comparisons = await Promise.all(eventIds.map(async eventId => {
        const [votes, payments, candidates, submissions] = await Promise.all([VoteRepository.getStatisticsByEvent(eventId), PaymentModel.getStatistics(eventId), CandidateRepository.getStatisticsByEvent(eventId), FormSubmissionRepository.getEventStatistics(eventId)]);
        return {
          eventId,
  ...
  async getConversionFunnel(eventId) {
    try {
      const [submissions, votes, payments] = await Promise.all([FormSubmissionRepository.getEventStatistics(eventId), VoteRepository.getStatisticsByEvent(eventId), PaymentModel.getStatistics(eventId)]);
      const funnel = {
        submissions: submissions.total,
  ...
  async getActivityHeatmap(eventId, startDate, endDate) {
    try {
      const timeline = await ActivityRepository.getTimeline(startDate, endDate, "hour", {
        event: eventId
      });
  ...
  async getRealTimeMetrics(eventId, minutes = 15) {
    try {
      ...
      const [voteVelocity, recentActivities] = await Promise.all([VoteRepository.getVoteVelocity(eventId, minutes), ActivityRepository.findByTimeRange(timeWindow, new Date(), {
        event: eventId
      }, 1, 100)]);
      return {
        voteVelocity,
  ...
  async getDeviceAnalytics(params = {}) {
    try {
      ...
      // Build event filter
      const eventQuery = params.event_id ? {
