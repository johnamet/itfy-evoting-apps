...
class AnalyticsController extends BaseController {
  ...
  async getDashboardOverview(req, res) {
    ...
    const overview = await this.service("analyticsService").getDashboardOverview({
      period
    });
    return this.success(res, {
  ...
  async getVotingTrends(req, res) {
    ...
    const trends = await this.service("analyticsService").getVotingTrends(eventId, interval || "day", start_date ? new Date(start_date) : undefined, end_date ? new Date(end_date) : undefined);
    return this.success(res, {
      data: trends
    });
  }
  ...
  async getRevenueTrends(req, res) {
    ...
    const trends = await this.service("analyticsService").getRevenueTrends(eventId, startDate, endDate, interval || "day");
    return this.success(res, {
      data: trends
    });
  }
  ...
  async getUserEngagement(req, res) {
    ...
    const endDate = end_date ? new Date(end_date) : new Date();
    const startDate = start_date ? new Date(start_date) : new Date(endDate.getTime() - 30 * 24 * 60 * 60 * 1000);
    const engagement = await this.service("analyticsService").getUserEngagement(eventId, startDate, endDate);
    return this.success(res, {
      data: engagement
    });
  }
  ...
  async getCandidateRanking(req, res) {
    ...
    const ranking = await this.service("analyticsService").getCandidateRanking(eventId, category_id || null, parseInt(limit, 10) || 20);
    return this.success(res, {
      data: ranking
    });
  }
  ...
  async getActivityHeatmap(req, res) {
    ...
    const endDate = end_date ? new Date(end_date) : new Date();
    const startDate = start_date ? new Date(start_date) : new Date(endDate.getTime() - 30 * 24 * 60 * 60 * 1000);
    const heatmap = await this.service("analyticsService").getActivityHeatmap(eventId, startDate, endDate);
    return this.success(res, {
      data: heatmap
    });
  }
  ...
  async getRealTimeMetrics(req, res) {
    ...
    const metrics = await this.service("analyticsService").getRealTimeMetrics(eventId, parseInt(minutes, 10) || 15);
    return this.success(res, {
      data: metrics
    });
  }
