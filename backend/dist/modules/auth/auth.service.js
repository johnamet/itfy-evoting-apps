...
import { ERROR_MESSAGES, MAX_LOGIN_ATTEMPTS, ACCOUNT_LOCK_DURATION_MINUTES, JWT_EXPIRATION } from "../../utils/constants/auth.constants.js";
...
class AuthService extends BaseService {
  ...
  async registerUser(data) {
    try {
      ...
      const verificationToken = await AuthHelpers.generateAndStoreVerificationToken(user._id.toString());
  ...
  async loginUser(data, metadata = {}) {
    try {
      ...
      const passwordHash = user?.password_hash || "$2b$10$X/invalid/hash/that/will/never/match/anythingXXXXXXXXXXXXXXXXX";
      ...
      if (metadata.userAgent) {
        await cache.set(`device:${user._id}`, JSON.stringify({
  ...
  async registerCandidate(data) {
    try {
      ...
      const existingCandidate = await this.candidateRepository.findByEmail(validated.email, validated.eventId);
      ...
      // Generate auth tokens
      const accessToken = AuthHelpers.generateToken(candidate._id.toString(), "candidate", "access");
      const refreshToken = AuthHelpers.generateToken(candidate._id.toString(), "candidate", "refresh");
  ...
  async loginCandidate(data, metadata = {}) {
    try {
      ...
      if (validated.identifier.toUpperCase().startsWith("CAN-")) {
        candidate = await this.candidateRepository.model.findOne({
      ...
      const passwordHash = candidate?.password_hash || "$2b$10$X/invalid/hash/that/will/never/match/anythingXXXXXXXXXXXXXXXXX";
      ...
      if (candidate.status !== STATUS.APPROVED && candidate.status !== STATUS.PROFILE_UPDATE_PENDING) {
        throw new Error(`Account is ${candidate.status}. Please contact the event administrator.`);
      }
      ...
      // Store device info if provided
      if (metadata.userAgent) {
        await cache.set(`device:candidate:${candidate._id}`, JSON.stringify({
          userAgent: metadata.userAgent,
          ip: metadata.ip,
          lastLogin: new Date()
        }), "EX", 30 * 24 * 60 * 60 // 30 days
  ...
  async changePassword(userId, data, userType = "user") {
    try {
      ...
      const isValidPassword = await AuthHelpers.comparePassword(validated.currentPassword, account.password_hash);
      if (!isValidPassword) {
  ...
  async logoutCandidate(candidateId, refreshToken) {
    return this.logout(candidateId, {
      refresh_token: refreshToken
    });
  }
  ...
  async resendVerification(data, userType = "user") {
    try {
      ...
      // Generate verification token
      const verificationToken = await AuthHelpers.generateAndStoreVerificationToken(account._id.toString());
