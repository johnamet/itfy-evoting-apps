...
async function withRetry(requestFn, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isRetryable = retryOn.includes(statusCode) || error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT';
...
class PaymentService extends BaseService {
  ...
  async initializePayment(paymentData) {
    try {
      ...
      if (couponCode) {
        ...
        const couponValidation = await this.couponService.validateCouponForBundle(couponCode, primaryBundle._id, voterEmail);
      ...
      const votesByCategory = Array.from(categoryVotesMap.values()).filter(item => item.category !== null);
      ...
      const paystackResponse = await this._makePaystackRequest('POST', '/transaction/initialize', paystackPayload);
  ...
  async initializePayment_deprecated(paymentData) {
    try {
      ...
      const paystackResponse = await axios.post(`${this.paystackBaseUrl}/transaction/initialize`, paystackPayload, {
        headers: {
          Authorization: `Bearer ${this.paystackSecretKey}`,
          "Content-Type": "application/json"
        }
  ...
  async verifyPayment(reference) {
    try {
      ...
      const paystackResponse = await axios.get(`${this.paystackBaseUrl}/transaction/verify/${reference}`, {
        headers: {
          Authorization: `Bearer ${this.paystackSecretKey}`
        }
      });
      ...
      const bundleNames = updatedPayment.bundles ? updatedPayment.bundles.map(b => b.bundle?.name || "Unknown").join(", ") : updatedPayment.bundle?.name || "Unknown Bundle";
  ...
  */
  async handleChargeSuccess(data) {
    try {
      ...
      if (!candidateId || !bundleCategoryMap || bundleCategoryMap.length === 0) {
        ...
        return {
          success: true,
          message: "Charge processed successfully - no auto-vote"
        };
      }
      ...
      if (payment.auto_vote_cast) {
        ...
        return {
          success: true,
          message: "Charge processed successfully - auto-vote already cast"
        };
      }
      ...
      try {
        ...
        await this.repository.updatePaymentById(payment._id, {
          auto_vote_cast: false,
          // Mark as not cast so it can be retried
          metadata: {
            ...payment.metadata,
            auto_vote_error: {
              failed: true,
  ...
  async processRefund(paymentId, reason, adminId) {
    try {
      ...
      const paystackResponse = await axios.post(`${this.paystackBaseUrl}/refund`, refundPayload, {
        headers: {
          Authorization: `Bearer ${this.paystackSecretKey}`,
          "Content-Type": "application/json"
        }
      ...
      await EmailService.sendEmail(payment.voter_email, "Refund Processed", "refund-processed",
      // Template name
      {
        voterName: payment.voter_name || payment.voter_email,
        eventName: payment.event.name,
