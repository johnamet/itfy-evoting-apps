...
class PaymentRepository extends BaseRepository {
  ...
  async getStatistics(eventId = null) {
    try {
      ...
      return {
        ...
        netRevenue: Math.round(((stats?.totalRevenue[0]?.total || 0) - (stats?.totalRefunded[0]?.total || 0)) * 100) / 100,
        totalVotesPurchased: stats?.totalVotesPurchased[0]?.total || 0,
        totalVotesCast: stats?.totalVotesCast[0]?.total || 0,
        totalVotesRemaining: (stats?.totalVotesPurchased[0]?.total || 0) - (stats?.totalVotesCast[0]?.total || 0),
  ...
  async exportPayments(eventId, filters = {}) {
    try {
      ...
      const payments = await this.model.find(query).populate("bundle", "name vote_count price").populate("coupon", "code discount_type discount_value").sort({
        created_at: -1
      }).lean().exec();
