...
class Payment extends BaseModel {
  constructor() {
    ...
    this.schema.pre("save", async function (next) {
      try {
        ...
        this.bundles.forEach(bundleItem => {
          bundleItem.votes_used = Math.min(bundleItem.votes_allocated, bundleItem.votes_allocated - (this.votes_remaining > 0 ? Math.min(this.votes_remaining, bundleItem.votes_allocated) : 0));
        });

        // Set confirmed_at when status changes to completed
        if (this.isModified("payment_status") && this.payment_status === STATUS.COMPLETED && !this.confirmed_at) {
          this.confirmed_at = new Date();
        }

        // Set failed_at when status changes to failed
        if (this.isModified("payment_status") && this.payment_status === STATUS.FAILED && !this.failed_at) {
          this.failed_at = new Date();
        }

        // Set refunded_at when status changes to refunded
        if (this.isModified("payment_status") && this.payment_status === STATUS.REFUNDED && !this.refunded_at) {
    ...
    this.schema.methods.getAvailableVotesForCategory = function (categoryId) {
      const categoryVotes = this.votes_by_category.find(vc => vc.category.toString() === categoryId.toString());
    ...
    this.schema.methods.useVotesForCategory = async function (categoryId, votesToUse = 1) {
      ...
      const categoryIndex = this.votes_by_category.findIndex(vc => vc.category._id.toString() === categoryId.toString());
      if (categoryIndex === -1) {
      ...
      for (const bundleSource of categoryVotes.bundle_sources) {
        ...
        if (toUse > 0) {
          ...
          const mainBundleIndex = this.bundles.findIndex(b => b.bundle.toString() === bundleSource.bundle.toString());
