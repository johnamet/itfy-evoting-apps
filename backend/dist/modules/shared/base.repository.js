...
class BaseRepository {
  ...
  _extractCacheContext(data) {
    ...
    const contextFields = ["eventId", "event_id", "categoryId", "category_id", "userId", "user_id", "candidateId", "candidate_id", "formId", "form_id"];
  ...
  async update(query, data, options = {}) {
    try {
      const updateQuery = this.model.findOneAndUpdate(query, {
        ...data,
        updated_at: Date.now()
      }, {
        new: true,
  ...
  async delete(query, options = {}) {
    try {
      const deleteQuery = this.model.findOneAndUpdate(query, {
        deleted_at: Date.now()
      }, {
        new: true
      });
  ...
  async restore(query, options = {}) {
    try {
      const q = this.model.findOneAndUpdate(query, {
        deleted_at: null
      }, {
        new: true
      });
