...
import { DEFAULT_PAGINATION, ERROR_MESSAGES } from "../../utils/constants/error.constants.js";
...
class BaseController {
  ...
  service(name) {
    ...
    if (!service) {
      throw new Error(`Service '${name}' not injected into ${this.constructor.name}. Available: ${Object.keys(this._services).join(", ") || "none"}`);
    }
    return service;
  }
  ...
  getPagination(req) {
    ...
    const limit = Math.min(Math.max(1, parseInt(req.query.limit, 10) || DEFAULT_PAGINATION.LIMIT), DEFAULT_PAGINATION.MAX_LIMIT);
