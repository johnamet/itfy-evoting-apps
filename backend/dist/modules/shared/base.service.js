...
class BaseService {
  ...
  async transaction(callback) {
    const repoWithTx = Object.values(this.repos).find(repo => repo && typeof repo.transaction === "function");
    if (!repoWithTx) {
  ...
  error(error, fallbackMessage = "Operation failed") {
    ...
    const isValidationError = error.message.includes("Validation failed") || error.message.includes("required") || error.message.includes("invalid");
  ...
  repo(name) {
    ...
    if (!repo) {
      throw new Error(`Repository '${name}' not injected into ${this.constructor.name}. Available: ${Object.keys(this.repos).join(", ")}`);
    }
    return repo;
  }
}
