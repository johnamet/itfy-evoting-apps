...
class ActivityService {
  ...
  async log({
    ...
    try {
      ...
      if (!severity) {
        if ([ACTION_TYPE.FAILED_LOGIN, ACTION_TYPE.SUSPICIOUS_ACTIVITY, ACTION_TYPE.PAYMENT_FAILED].includes(action)) {
  ...
  async logAuth(userId, action, ipAddress, userAgent, success = true) {
    ...
    return await this.log({
      ...
      entityId: userId,
      description: success ? `User ${action} successfully` : `Failed login attempt for user`,
      severity,
      ipAddress,
      userAgent
    });
  ...
  async logPayment(userId, paymentId, eventId, action, metadata) {
    const severity = action === ACTION_TYPE.PAYMENT_FAILED ? SEVERITY.ERROR : SEVERITY.INFO;
    return await this.log({
      userId,
      action,
  ...
  async generateAuditReport(startDate, endDate, filters = {}) {
    const [activities, actionStats, timeline] = await Promise.all([ActivityRepository.findByTimeRange(startDate, endDate, filters, 1, 1000), ActivityRepository.getActionStatistics(startDate, endDate), ActivityRepository.getTimeline(startDate, endDate, "day", filters)]);
    return {
      period: {
        startDate,
        endDate
