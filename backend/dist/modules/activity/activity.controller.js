...
class ActivityController extends BaseController {
  ...
  async list(req, res) {
    ...
    const filters = this.getFilters(req, ["user", "action", "entity_type", "entity_id", "event", "severity", "session_id", "ip_address"]);
    ...
    const result = await this.service("activityService").repository.findAll(filters, page, limit, {
      sort,
      populate: ["user", "event"]
    });
  ...
  async getTimeline(req, res) {
    ...
    const timeline = await this.service("activityService").getActivityTimeline(eventId, interval || "day", start_date ? new Date(start_date) : undefined, end_date ? new Date(end_date) : undefined);
    return this.success(res, {
      data: timeline
    });
  }
  ...
  async getRecent(req, res) {
    ...
    const result = await this.service("activityService").repository.findAll(filters, 1, limit, {
      sort: {
        timestamp: -1
      },
      populate: ["user", "event"]
