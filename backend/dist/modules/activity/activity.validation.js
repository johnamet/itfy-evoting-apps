...
export const activityQuerySchema = Joi.object({
  ...
  sort: Joi.string().valid("timestamp", "action", "severity", "-timestamp", "-action", "-severity").default("-timestamp")
});
...
export const logAuthActivitySchema = Joi.object({
  user_id: ObjectId().required(),
  action: Joi.string().valid(ACTION_TYPE.LOGIN, ACTION_TYPE.LOGOUT, ACTION_TYPE.FAILED_LOGIN, ACTION_TYPE.PASSWORD_RESET).required(),
