...
class EventService extends BaseService {
  ...
  async cloneEvent(eventId, adminId, overrides = {}) {
    try {
      const originalEvent = await this.repository.findById(eventId, {
        lean: true
      });
      if (!originalEvent) {
        throw new Error("Event not found");
  ...
  async isRegistrationOpen(eventId) {
    try {
      ...
      return event.is_published && event.registration_enabled && now >= new Date(registrationStart) && now <= new Date(registrationEnd) && event.current_attendees < event.max_attendees;
    } catch (error) {
  ...
  async cancelEvent(eventId, reason, adminId) {
    try {
      ...
      NotificationService.notifyEventCancelled(eventId, reason).catch(err => console.error("Notification error:", err));
  ...
  async getEventResults(eventId) {
    try {
      ...
      const results = await Promise.all(categoriesResult.data.map(async category => {
