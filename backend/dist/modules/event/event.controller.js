...
import { createEventSchema, updateEventSchema, updateEventStatusSchema } from "./event.validation.js";
class EventController extends BaseController {
  ...
  async list(req, res) {
    ...
    const filters = this.getFilters(req, ["status", "is_published", "is_featured", "event_type", "visibility", "created_by"]);
    ...
    const [events, total] = await Promise.all([this.service("eventService").listEvents(filters, {
      skip,
      limit,
      sort
  ...
  async getUpcoming(req, res) {
    ...
    const events = await this.service("eventService").getUpcomingEvents(limit, {
      skip
    });
  ...
  async getCandidates(req, res) {
    ...
    const [candidates, total] = await Promise.all([this.service("eventService").getEventCandidates(id, filters, {
      skip,
      limit
  ...
  async adminList(req, res) {
    ...
    const filters = this.getFilters(req, ["status", "is_published", "is_featured", "created_by"]);
    const sort = this.getSort(req, "-created_at");
    const search = this.getSearch(req);
    if (search) {
      filters.$or = [{
