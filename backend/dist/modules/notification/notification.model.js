...
NotificationSchema.virtual("canRetry").get(function () {
  return this.status === NOTIFICATION_STATUS.FAILED && this.retry_count < this.max_retries && (!this.next_retry_at || new Date() >= this.next_retry_at);
});
...
NotificationSchema.statics.findPendingForDelivery = async function (channel, limit = 100) {
  return await this.find({
    channel,
    status: {
      $in: [NOTIFICATION_STATUS.PENDING, NOTIFICATION_STATUS.QUEUED]
    },
    deleted_at: null
  }).sort({
    priority: -1,
      ...
      deliveryRate: [{
        $group: {
          ...
          delivered: {
            $sum: {
              $cond: [{
                $eq: ["$status", NOTIFICATION_STATUS.DELIVERED]
              }, 1, 0]
            }
          },
          failed: {
            $sum: {
              $cond: [{
                $eq: ["$status", NOTIFICATION_STATUS.FAILED]
              }, 1, 0]
            }
          }
        }
