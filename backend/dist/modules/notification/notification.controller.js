...
class NotificationController extends BaseController {
  ...
  async list(req, res) {
    ...
    const filters = this.getFilters(req, ["user", "type", "channel", "status", "priority", "event", "batch_id"]);
    ...
    const result = await this.service("notificationRepository").findAll(filters, page, limit, {
      sort,
      populate: ["user", "event"]
    });
  ...
  async bulkMarkAsRead(req, res) {
    ...
    const results = await Promise.allSettled(validated.notification_ids.map(id => this.service("notificationRepository").markAsRead(id)));
  ...
  async getByChannel(req, res) {
    ...
    const notifications = await this.service("notificationRepository").findByChannel(channel, status || null, limit);
    return this.success(res, {
      data: notifications
    });
  }
  ...
  async getPending(req, res) {
    ...
    const limit = parseInt(req.query.limit, 10) || 100;
    const notifications = await this.service("notificationRepository").findPendingForDelivery(channel, limit);
    return this.success(res, {
      data: notifications
    });
  }
  ...
  async markAsSent(req, res) {
    ...
    const notification = await this.service("notificationRepository").markAsSent(id, provider_id || null);
    return this.success(res, {
      data: notification,
  ...
  async markAsFailed(req, res) {
    ...
    const notification = await this.service("notificationRepository").markAsFailed(id, error_message, should_retry !== false);
    return this.success(res, {
      data: notification,
  ...
  async bulkCreate(req, res) {
    ...
    const results = await Promise.allSettled(user_ids.map(userId => this.service("notificationRepository").create({
      ...notificationData,
      user: userId
    })));
    const succeeded = results.filter(r => r.status === "fulfilled").length;
