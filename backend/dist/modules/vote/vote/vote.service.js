...
class VoteService extends BaseService {
    ...
    try {
      ...
      if (availableVotes <= 0) {
        ...
        const canVoteInCategory = payment.bundles.some(b => {
          ...
          return b.applicable_categories.some(cat => cat.toString() === categoryId.toString());
        });
      ...
      await agendaManager.now("send-vote-confirmation-email", {
        ...
        categoryVotesRemaining: votingSummary.categories.find(c => c.category_id.toString() === categoryId.toString())?.votes_remaining || 0,
      ...
      this.activityService.logVote(null, vote._id, payment.event, {
        ...
        categoryName: populatedVote.category.name,
        votesRemaining: votingSummary.votes_remaining
      }, ipAddress).catch(err => console.error("Activity log error:", err));
      return {
  ...
  async castAggregatedVotes(params) {
    try {
      ...
      for (const mapping of bundleCategoryMap) {
        ...
        // Find bundle data
        const bundleData = payment.bundles.find(b => b.bundle._id.toString() === bundleId.toString());
        if (!bundleData) {
  ...
  async aggregateVoteCounts(eventId) {
    try {
      ...
      const candidateUpdates = await Promise.all(candidateCounts.map(async ({
        _id,
        vote_count
      }) => {
        return this.candidateRepository.updateById(_id, {
          vote_count
        });
      }));

      // Update category vote counts
      const categoryUpdates = await Promise.all(categoryCounts.map(async ({
        _id,
        total_votes
      }) => {
        return this.categoryRepository.updateById(_id, {
          total_votes
        });
      }));
      const result = {
