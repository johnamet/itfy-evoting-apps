...
class BundleService extends BaseService {
  ...
  async deleteBundle(bundleId, adminId) {
    try {
      ...
      const deletedBundle = await this.repository.delete({
        _id: bundleId
      });
  ...
  async activateBundle(bundleId, adminId) {
    try {
      ...
      const activatedBundle = await this.repository.updateById(bundleId, {
  ...
  async deactivateBundle(bundleId, adminId) {
    try {
      ...
      const deactivatedBundle = await this.repository.updateById(bundleId, {
  ...
  async reorderBundles(eventId, orderData, adminId) {
    try {
      ...
      const updatePromises = orderData.map(item => this.repository.updateById(item.bundleId, {
  ...
  async cloneBundle(bundleId, overrides = {}, adminId) {
    try {
      ...
      const cloneData = {
        ...
        is_featured: false,
        is_popular: false,
        status: overrides.status || STATUS.INACTIVE,
  ...
  async getBundlePerformance(eventId) {
    try {
      const bundles = await this.repository.findByEvent(eventId, {
        lean: true
      });
      const performanceData = await Promise.all(bundles.map(async bundle => {
