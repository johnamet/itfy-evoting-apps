...
class BundleController extends BaseController {
  ...
  async list(req, res) {
    ...
    const result = await this.service("bundleService").repository.findAll(filters, page, limit, {
      sort
    });
  ...
  async getByPriceRange(req, res) {
    ...
    const bundles = await this.service("bundleService").getBundlesByPriceRange(parseFloat(min_price), parseFloat(max_price), eventId || null);
    return this.success(res, {
      data: bundles
    });
  }
