...
class CouponService extends BaseService {
  ...
  async deleteCoupon(couponId, adminId) {
    try {
      ...
      const deletedCoupon = await this.repository.delete({
        _id: couponId
      });
  ...
  async activateCoupon(couponId, adminId) {
    try {
      ...
      const activatedCoupon = await this.repository.updateById(couponId, {
  ...
  async deactivateCoupon(couponId, adminId) {
    try {
      ...
      const deactivatedCoupon = await this.repository.updateById(couponId, {
  ...
  async validateCouponForBundle(code, bundleId, userEmail) {
    try {
      ...
      if (userEmail) {
        const userRedemptions = await this.repository.getUserRedemptionCount(coupon._id, userEmail);
      ...
      if (coupon.applicable_bundles && coupon.applicable_bundles.length > 0) {
        const isApplicable = coupon.applicable_bundles.some(bundle => bundle.toString() === bundleId);
        if (!isApplicable) {
          return {
            isValid: false,
