...
class SlideService extends BaseService {
  ...
  async createSlide(slideData, adminId) {
    try {
      ...
      if (!validatedData.display_order && validatedData.display_order !== 0) {
        const existingSlides = await this.repository.findByType(validatedData.slide_type || SLIDE_TYPE.HERO);
  ...
  async deleteSlide(slideId, adminId, deleteImages = false) {
    try {
      ...
      const deletedSlide = await this.repository.delete({
        _id: slideId
      });
  ...
  async reorderSlides(orderData, slideType, adminId) {
    try {
      ...
      const updatePromises = orderData.map(item => this.repository.updateById(item.slideId, {
  ...
  async getAllSlides(page = 1, limit = 10, filters = {}, options = {}) {
    try {
      const slides = await this.repository.findAll(filters, page, limit, {
        ...options,
        sort: {
          display_order: 1,
          created_at: -1
  ...
  async cloneSlide(slideId, overrides = {}, adminId) {
    try {
      ...
      const cloneData = {
        ...
        status: overrides.status || SLIDE_STATUS.DRAFT,
  ...
  async bulkUpdateStatus(slideIds, status, adminId) {
    try {
      ...
      // Update all slides
      const updatePromises = slideIds.map(slideId => this.repository.updateById(slideId, {
        status
      }));
      const updatedSlides = await Promise.all(updatePromises);
