...
import { SLIDE_STATUS, SLIDE_TYPE, SLIDE_POSITION, ANIMATION_TYPE, BUTTON_STYLE } from "../../utils/constants/slide.constants.js";
  ...
  constructor() {
    ...
    this.schema.pre("save", async function (next) {
      try {
        ...
        if (this.status === SLIDE_STATUS.SCHEDULED && this.start_date && new Date() >= this.start_date) {
          this.status = SLIDE_STATUS.ACTIVE;
        }

        // Auto-deactivate slides that have passed their end date
        if (this.status === SLIDE_STATUS.ACTIVE && this.end_date && new Date() > this.end_date) {
          this.status = SLIDE_STATUS.INACTIVE;
        }
        next();
      } catch (error) {
