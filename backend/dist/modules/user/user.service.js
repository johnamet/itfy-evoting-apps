...
class UserService extends BaseService {
  ...
  async updateUserRole(userId, newRole, adminId) {
    try {
      ...
      const {
        ...
        role: newRole
      }, UserValidation.updateRoleSchema);
      const user = await this.repository.findById(userId);
      if (!user) {
        throw new Error("User not found");
      }
  ...
  async deactivateUser(userId, adminId, reason = null) {
    try {
      ...
      const validated = this.validate({
        reason
      }, UserValidation.deactivateUserSchema);
      const user = await this.repository.findById(userId);
      if (!user) {
        throw new Error("User not found");
      }
  ...
  async searchUsers(searchTerm, page = 1, limit = 20, options = {}) {
    try {
      ...
      return await this.repository.searchUsers(validated.search_term, validated.page, validated.limit, options);
    } catch (error) {
  ...
  async getUserStats() {
    try {
      ...
      const inactiveUsers = await this.repository.count({
        status: STATUS.INACTIVE
      });
      const suspendedUsers = await this.repository.count({
        status: STATUS.SUSPENDED
      });

      // Get counts by role
      const superAdmins = await this.repository.count({
        role: ROLES.SUPER_ADMIN
      });
      const admins = await this.repository.count({
        role: ROLES.ADMIN
      });
      const moderators = await this.repository.count({
        role: ROLES.MODERATOR
      });
      const voters = await this.repository.count({
        role: ROLES.VOTER || ROLES.USER
      });
      ...
      const unverifiedUsers = await this.repository.count({
  ...
  async getUserStatistics(userId) {
    try {
      ...
      // Get activity count
      const activityCount = await ActivityService.repository.count({
        userId
      });
  ...
  */
  ...
  exportToCSV(users) {
    ...
    const rows = users.map(user => [user._id, user.name, user.email, user.role, user.status, user.email_verified ? "Yes" : "No", new Date(user.created_at).toISOString()]);
