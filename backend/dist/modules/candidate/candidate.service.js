...
class CandidateService extends BaseService {
  ...
  _convertImagesToUrls(candidate) {
    ...
    const candidateData = candidate.toObject ? candidate.toObject() : {
      ...candidate
    };
  ...
  async findAll(filters = {}, options = {}) {
    try {
      ...
      const query = this.repository.model.find(filters).skip(skip).limit(limit).sort(sort);
  ...
  async getCandidateBySlug(slug) {
    try {
      const candidate = await this.repository.model.findOne({
        slug
  ...
  async updateProfileByCandidate(candidateId, updateData) {
    try {
      ...
      const forbiddenFields = attemptedFields.filter(field => ADMIN_ONLY_FIELDS.includes(field));
      if (forbiddenFields.length > 0) {
        throw new Error(`Cannot update admin-only fields: ${forbiddenFields.join(", ")}`);
      }
      ...
      if (candidate.status === STATUS.APPROVED || candidate.status === STATUS.PENDING) {
        await this.repository.requestReApproval(candidateId, changedFields, PROFILE_UPDATE_REASONS.CORRECTION);
  ...
  async addCategory(candidateId, categoryId) {
    try {
      ...
      const validated = this.validate({
        categoryId
      }, CandidateValidation.addCategorySchema);
      const candidate = await this.repository.findById(candidateId);
      if (!candidate) {
        throw new Error("Candidate not found");
      }
      ...
      if (candidate.status === STATUS.APPROVED || candidate.status === STATUS.PROFILE_UPDATE_PENDING) {
        await this.repository.requestReApproval(candidateId, ["categories"], PROFILE_UPDATE_REASONS.CATEGORY_CHANGE);
  ...
  async updateAdminVerifiedCategories(candidateId, categoryIds) {
    try {
      ...
      const validated = this.validate({
        categoryIds
      }, CandidateValidation.updateAdminVerifiedCategoriesSchema);
  ...
  async rejectProfile(candidateId, adminId, reason = "") {
    try {
      ...
      const validated = this.validate({
        reason
      }, CandidateValidation.rejectCandidateSchema);
  ...
  async search(query, filters = {}, page = 1, limit = 20, options = {}) {
    try {
      ...
      const validated = this.validate({
        ...
        page,
        limit
      }, CandidateValidation.searchSchema);
      return await this.repository.search(validated.query, validated.filters, validated.page, validated.limit, options);
    } catch (error) {
  ...
  async createFromNomination(nominationData, eventId, categoryIds, submissionId, adminId = null) {
    try {
      ...
      // Generate slug
      const slug = await this.generateUniqueSlug(`${validatedData.first_name}-${validatedData.last_name}`);
  ...
  async sendWelcomeEmail(candidateId) {
    try {
      ...
      if (!temporaryPassword) {
        ...
        const passwordHash = await AuthHelpers.hashPassword(temporaryPassword);
        await this.repository.updateById(candidateId, {
          password_hash: passwordHash
        });
      }
  ...
  async generateUniqueSlug(baseName) {
    let slug = baseName.toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").trim();
    let exists = true;
    let counter = 0;
    while (exists) {
  ...
  async getCandidateStats(candidateId) {
    try {
      ...
      const candidatesInCategories = await this.repository.model.find({
        categories: {
          $in: categoryIds
        },
  ...
  async getCandidateVotes(candidateId) {
    try {
      const VoteRepository = (await import("../vote/vote/vote.repository.js")).default;
      const [totalVotes, recentVotes] = await Promise.all([VoteRepository.countForCandidate(candidateId), VoteRepository.findByCandidate(candidateId, {
        limit: 10,
        sort: {
          cast_at: -1
        },
  ...
  async incrementViewCount(candidateId) {
    try {
      await this.repository.model.findByIdAndUpdate(candidateId, {
        $inc: {
          view_count: 1
        }
      }, {
  ...
  async removeGalleryImage(candidateId, imageUrl) {
    try {
      ...
      const updatedGallery = (candidate.gallery || []).filter(img => img !== imageUrl);
