...
class Candidate extends BaseModel {
  constructor() {
    ...
    this.schema.pre("save", async function (next) {
      try {
        ...
        if (!this.slug && this.first_name && this.last_name) {
          const baseSlug = `${this.first_name}-${this.last_name}`.toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").trim();
    ...
    this.schema.methods.removeFromCategory = async function (categoryId) {
      ...
      const isAdminVerified = this.admin_verified_categories.some(cat => cat.toString() === categoryId.toString());
      if (isAdminVerified) {
        throw new Error("Cannot remove admin-verified category. Please contact administrator.");
      }
      const index = this.categories.findIndex(cat => cat.toString() === categoryId.toString());
      if (index === -1) {
