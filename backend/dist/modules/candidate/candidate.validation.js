...
class CandidateValidation {
  ...
  static createCandidateSchema = Joi.object({
    ...
    bio: Joi.string().trim().max(2000).optional().messages({
    ...
    why_nominate_me: Joi.string().trim().max(1000).optional().messages({
      "string.max": "Why nominate me cannot exceed 1000 characters"
    }),
    impact_statement: Joi.string().trim().max(1000).optional().messages({
  ...
  static updateProfileSchema = Joi.object({
    first_name: Joi.string().trim().min(2).max(100).messages({
      "string.min": "First name must be at least 2 characters",
      "string.max": "First name cannot exceed 100 characters"
    }),
    last_name: Joi.string().trim().min(2).max(100).messages({
      "string.min": "Last name must be at least 2 characters",
      "string.max": "Last name cannot exceed 100 characters"
    }),
    phone_number: Joi.string().trim(),
    bio: Joi.string().trim().max(2000).messages({
      "string.max": "Bio cannot exceed 2000 characters"
    }),
    ...
    social_links: socialLinksSchema,
    why_nominate_me: Joi.string().trim().max(1000).messages({
      "string.max": "Why nominate me cannot exceed 1000 characters"
    }),
    impact_statement: Joi.string().trim().max(1000).messages({
      "string.max": "Impact statement cannot exceed 1000 characters"
    }),
  ...
  static updateAdminVerifiedCategoriesSchema = Joi.object({
    categoryIds: Joi.array().items(ObjectId()).min(1).required().messages({
      "array.min": "At least one category is required",
  ...
  static candidateQuerySchema = Joi.object({
    ...
    sort: Joi.string().valid("first_name", "last_name", "vote_count", "view_count", "nomination_date", "created_at", "-first_name", "-last_name", "-vote_count", "-view_count", "-nomination_date", "-created_at").default("-created_at")
  });
  ...
  static incrementVoteCountSchema = Joi.object({
    increment: Joi.number().integer().min(1).default(1).messages({
