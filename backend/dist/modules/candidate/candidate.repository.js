...
class CandidateRepository extends BaseRepository {
  ...
  async approve(candidateId) {
    try {
      ...
      await this.updateById(candidateId, {
        status: STATUS.APPROVED
      });
  ...
  async findProfileUpdatesPending(eventId = null, options = {}) {
    try {
      const filter = {
        status: STATUS.PROFILE_UPDATE_PENDING
      };
      if (eventId) filter.event = eventId;
      const query = this.model.find(filter).sort({
        updated_at: -1
  ...
  async adminRemoveCategory(candidateId, categoryId) {
    try {
      ...
      candidate.categories = candidate.categories.filter(cat => cat.toString() !== categoryId.toString());
      candidate.admin_verified_categories = candidate.admin_verified_categories.filter(cat => cat.toString() !== categoryId.toString());
