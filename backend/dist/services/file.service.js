...
import { MAX_FILE_SIZE_MB, FILE_SIZE_LIMITS, ALLOWED_FILE_TYPES, UPLOAD_DIRECTORIES, IMAGE_OPTIMIZATION, FILE_VALIDATION, CLOUD_STORAGE, CLEANUP_SETTINGS, FILE_ERROR_CODES } from "../utils/constants/file.constants.js";
...
class FileService {
  ...
  async initializeDirectories() {
    const directories = Object.values(UPLOAD_DIRECTORIES).map(dir => path.join(this.uploadDir, dir));
    try {
  ...
  isPathSafe(filePath) {
    ...
    return resolvedPath.startsWith(uploadBasePath + path.sep) || resolvedPath === uploadBasePath;
  }
  ...
  async validateFileMagicNumber(filePath, claimedMimeType) {
    try {
      ...
      if (!expectedSignatures) {
        ...
        logger.warn('Unknown MIME type for magic number validation', {
          claimedMimeType
        });
        return {
          isValid: true,
  ...
  imageFilter(req, file, cb) {
    if (ALLOWED_FILE_TYPES.IMAGES.includes(file.mimetype)) {
      ...
      cb(new Error(`Invalid image type. Allowed types: ${allowedTypes}`), false);
    }
  }
  ...
  async validateImageDimensions(filePath, validationType = "PROFILE") {
    try {
      ...
      if (!isValid) {
        throw new Error(`Image dimensions too small. Minimum: ${rules.width}x${rules.height}px, ` + `got: ${metadata.width}x${metadata.height}px`);
      }
      return true;
    } catch (error) {
  ...
  async deleteFile(filePath) {
    ...
    try {
      ...
      const absolutePath = path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath.replace(/^\//, ""));
  ...
  async cleanupOrphanedFiles(referencedFiles = [], directory = null) {
    try {
      const directories = directory ? [directory] : Object.values(UPLOAD_DIRECTORIES).filter(d => d !== UPLOAD_DIRECTORIES.TEMP);
      let deletedCount = 0;
      for (const dir of directories) {
        ...
        for (const file of files) {
          ...
          const isReferenced = referencedFiles.some(ref => ref.includes(file) || ref.includes(relativeFilePath));
          if (!isReferenced) {
