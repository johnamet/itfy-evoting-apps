  ...
  _defineJobs() {
    ...
    this.agenda.define("aggregate-vote-counts", {
      ...
      try {
        ...
        const {
          default: VoteService
        } = await import("../modules/vote/vote/vote.service.js");
    ...
    this.agenda.define("detect-fraud-patterns", {
      ...
      try {
        ...
        } = await import("../modules/activity/activity.service.js");
    ...
    this.agenda.define("send-vote-confirmation-email", {
      ...
      try {
        ...
        const {
          default: VoteRepository
        } = await import("../../modules/vote/vote/vote.repository.js");
    ...
    this.agenda.define("send-event-reminder-emails", {
      ...
      try {
        ...
        } = await import("../modules/event/event.repository.js");
        const {
    ...
    this.agenda.define("send-admin-profile-update-alert-email", {
      ...
      try {
        ...
        await EmailService.sendAdminProfileUpdateAlertEmail(adminEmail, candidateName, eventTitle, changedFields);
     ...
     */
    this.agenda.define("activate-starting-events", {
      priority: "high",
      concurrency: 1
    }, async () => {
      try {
        ...
        const {
          STATUS
        } = await import("../utils/constants/event.constants.js");
        const now = new Date();
        ...
        const eventsToActivate = await EventRepository.findAll({
          status: {
            $in: [STATUS.UPCOMING, STATUS.PENDING]
          },
          start_date: {
            $lte: now
          },
     ...
     */
    this.agenda.define("archive-expired-events", {
      priority: "normal",
      concurrency: 1
    }, async () => {
      try {
        ...
        for (const event of expiredEvents.data) {
          ...
          await this.now("close-event-cascade", {
            eventId: event._id
          });
          archivedCount++;
        }
     ...
     */
    this.agenda.define("send-event-started-notification", {
      priority: "normal",
      concurrency: 10
    }, async job => {
      const {
     ...
     */
    this.agenda.define("check-registration-deadlines", {
      priority: "normal",
      concurrency: 1
    }, async () => {
      try {
        ...
        const events = await EventRepository.findAll({
          status: {
            $in: [STATUS.UPCOMING, STATUS.ACTIVE]
          },
     ...
     */
    this.agenda.define("open-category-voting", {
      priority: "high",
      concurrency: 1
    }, async () => {
      try {
        ...
        } = await import("../modules/category/category.repository.js");
        const {
          STATUS
        } = await import("../utils/constants/category.constants.js");
        const now = new Date();
     ...
     */
    this.agenda.define("close-category-voting", {
      priority: "high",
      concurrency: 1
    }, async () => {
      try {
     ...
     */
    this.agenda.define("sync-category-status-with-events", {
      priority: "low",
      concurrency: 1
    }, async () => {
      try {
        ...
        for (const event of archivedEvents.data) {
          ...
          const activeCategories = await CategoryRepository.findAll({
            event: event._id,
            status: {
              $nin: [CATEGORY_STATUS.ARCHIVED, CATEGORY_STATUS.DELETED]
            }
          }, 1, 500);
     ...
     */
    this.agenda.define("send-voting-started-notification", {
      priority: "normal",
      concurrency: 10
    }, async job => {
      const {
     ...
     */
    this.agenda.define("send-voting-ended-notification", {
      priority: "normal",
      concurrency: 10
    }, async job => {
      const {
     ...
     */
    this.agenda.define("aggregate-category-votes", {
      priority: "normal",
      concurrency: 5
    }, async job => {
      const {
      ...
      try {
        ...
        const {
          default: VoteRepository
        } = await import("../modules/vote/vote.repository.js");
        const {
          default: CategoryRepository
        } = await import("../modules/category/category.repository.js");
     ...
     */
    this.agenda.define("send-category-deadline-reminders", {
      priority: "normal",
      concurrency: 1
    }, async () => {
      try {
    ...
    this.agenda.define("close-event-cascade", {
      ...
      try {
        ...
        await this.now("publish-event-results", {
          eventId
        });
        ...
        await this.schedule("archive-event-candidates", {
          eventId
        }, oneWeekFromNow);
    ...
    this.agenda.define("archive-event-candidates", {
      ...
      try {
        ...
        const {
          default: CandidateRepository
        } = await import("../modules/candidate/candidate.repository.js");
    ...
    this.agenda.define("cleanup-expired-tokens", {
      ...
      try {
        ...
        const {
          cache: _cache
        } = await import("../utils/cache/cache.utils.js");
    ...
    this.agenda.define("log-activity", {
      ...
      try {
        ...
        } = await import("../modules/activity/activity.repository.js");
    ...
    this.agenda.define("detect-suspicious-activities", {
      ...
      try {
        ...
        const suspiciousActivities = await ActivityRepository.detectSuspiciousPatterns(threshold, thresholdMinutes);
